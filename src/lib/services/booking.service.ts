 ...
 * asegurando la combinación correcta del código de país y móvil de Argentina (549)
 * y eliminando estrictamente espacios, guiones, signos +, paréntesis y caracteres especiales con regex,
 * retornando exclusivamente un string continuo de dígitos numéricos para la URL de wa.me (ej: '5492604614537').
 */
export function sanitizeWhatsAppPhone(phone: string): string {
  if (!phone) return '';
  // 1. Extraer exclusivamente los dígitos eliminando cualquier caracter no numérico
  let digits = phone.replace(/\D/g, '');
  if (!digits) return '';

  // 2. Si ya contiene el código internacional de Argentina y móvil (549...)
  if (digits.startsWith('549')) {
    return digits;
  }
  // 3. Si contiene el código de país sin el 9 (ej: 54 260...), quitar el 54 para anteponer 549
  if (digits.startsWith('54')) {
    digits = digits.slice(2);
  }
  // 4. Quitar ceros a la izquierda (código de área interurbano, ej: 0260 -> 260)
  digits = digits.replace(/^0+/, '');

  // 5. Combinar el prefijo oficial de Argentina para WhatsApp: 549 + números ingresados
  const combined = `549${digits}`;

  // 6. Limpieza final con regex para garantizar 100% que sea un string continuo numérico
 ...
 * y el número de teléfono del usuario dinámicamente sanitizado y combinado con +54 9.
 */
...
export class BookingService {
  static async createBooking(dto: CreateBookingDTO, appBaseUrl: string) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      // c. Doble chequeo de concurrencia dentro de la transacción (San Rafael UTC-3)
      ...
      // e. Crear Registro de Turno normalizado en UTC
      const appointmentTargetDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], 12, 0, 0));
